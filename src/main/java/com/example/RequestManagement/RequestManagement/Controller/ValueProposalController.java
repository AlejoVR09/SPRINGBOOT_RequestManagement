...
import com.example.RequestManagement.RequestManagement.Service.ValueProposalService;
...
@RestController
@RequestMapping("api/v1/valueProposal")
public class ValueProposalController {
    @Autowired
    ValueProposalService service;

    @PostMapping
    public ResponseEntity<?> save(@RequestBody ValueProposal valueProposal){
        try {
            return ResponseEntity
                    .status(HttpStatus.ACCEPTED)
                    .body(this.service.add(valueProposal));
        }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> modify(@RequestBody ValueProposal valueProposal ,@PathVariable Integer id){
        try {
            return ResponseEntity
                    .status(HttpStatus.ACCEPTED)
                    .body(this.service.modify(id, valueProposal));
        }
        catch (RuntimeException error){
            RepresentantErrorDTO representantErrorDTO = new RepresentantErrorDTO();
            representantErrorDTO.setMessage(error.getMessage());
